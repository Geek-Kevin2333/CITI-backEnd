...
public class RoleController {
	...
	public ResultVO<Boolean> save(@RequestBody Role Role){
		...
		else return new ResultVO<Boolean>(StatusCode.BAD_REQUEST,Role.getComment()+"插入失败",false);
	}
	...
	public ResultVO<Boolean> update(@RequestBody Role Role){
		...
		else return new ResultVO<Boolean>(StatusCode.BAD_REQUEST,Role.getComment()+"更新失败",false);
	}
	...
	public ResultVO<List<Role>> getAll(){
		...
		else return new ResultVO<List<Role>>(StatusCode.BAD_REQUEST,"查找失败",null);
	}
}
